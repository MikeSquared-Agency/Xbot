...
async def run_daily() -> None:
    ...
    try:
        ...
        replies = await collect_reply_window(conn)
        if len(replies) < 5:
            console.print(
        ...
        await update_strategy_weights(conn, analysis.strategy_stats)

        # Phase 4b: Maybe evolve voice profile
        try:
            thirty_day_avg = await get_thirty_day_avg(conn)
            evolved = await maybe_evolve_voice(conn, analysis, thirty_day_avg)
            if evolved:
        ...
        if digest:
            await store_digest(conn, analysis, digest)
