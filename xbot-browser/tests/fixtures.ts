...
export const test = baseTest.extend<TestFixtures & TestOptions, WorkerFixtures>({
  ...
  startClient: async ({ mcpHeadless, mcpBrowser, mcpArgs }, use, testInfo) => {
    ...
    await use(async options => {
      ...
      const { transport, stderr } = await createTransport(args, testInfo.outputPath('ms-playwright'), options?.extensionToken);
      let stderrBuffer = '';
      stderr?.on('data', data => {
...
async function createTransport(args: string[], profilesDir: string, extensionToken?: string): Promise<{
  transport: Transport,
  stderr: Stream | null,
}> {
